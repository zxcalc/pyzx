...
### Changed
- Migrated project configuration from `setup.py` to `pyproject.toml`. The `galois` package is now an optional dependency (install via `pip install "pyzx[web]"` or `pip install "pyzx[all]"`), as it is only used in `pyzx.web` which is not imported by the main package.
