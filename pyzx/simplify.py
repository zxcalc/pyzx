...
Uses ``simp_override`` so that a whole-graph batch matcher runs each step; applying
gadget pivots one at a time would let gadget vertices created by an earlier pivot
participate in later matches."""
...
Uses ``simp_override`` for the same reason as :data:`pivot_gadget_simp`."""
...
def reduce_scalar(g: BaseGraph[VT,ET], quiet:bool=True, stats:Optional[Stats]=None) -> int:
    ...
    It skips the boundary pivots. Input must already be in graph-like form."""
    i = 0
    while True:
        i1 = id_simp(g)
        i2 = spider_simp(g)
