...
synonyms_hadamard = ['Hadamard', 'hadamard', 'H']

synonyms_edge = ['empty', 'simple', 'none']
synonyms_hedge = ['hadamard edge', 'Hadamard edge']

tikz_error_message = "Not a valid tikz picture. Please use Tikzit to generate correct output."
def tikz_to_graph(s: str, warn_overlap:bool= True, backend:Optional[str]=None):
    ...
    If warn_overlap is True, this method gives warnings when two vertices have the same position
    (which might point towards an error in the tikz).
    """
    lines = [l.strip() for l in s.strip().splitlines() if l.strip() != '']
    if lines[0] != r'\begin{tikzpicture}':
    ...
    for c,l in enumerate(lines[2:]):
        ...
        ty: VertexType.Type
        if style in synonyms_boundary: ty = VertexType.BOUNDARY
        elif style in synonyms_z: ty = VertexType.Z
        elif style in synonyms_x: ty = VertexType.X
        elif style in synonyms_hadamard: ty = VertexType.H_BOX
        else:
