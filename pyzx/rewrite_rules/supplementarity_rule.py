...
from pyzx.utils import  phase_is_clifford
from pyzx.graph.base import BaseGraph, VT, ET

MatchSupplementarityType = Tuple[VT, VT, Literal[1, 2], FrozenSet[VT]]

def match_supplementarity(g: BaseGraph[VT, ET], v: VT, w: VT) -> List[MatchSupplementarityType[VT]]:
    ...
    if phase_is_clifford(phases[v]): return False  # Skip Clifford vertices
    neigh = set(g.neighbors(v))
    if not neigh.isdisjoint(taken): return False
    par = frozenset(neigh)
    ...
    if par in parities:
        for w in parities[par]:
            ...
            parities[par].append(v)
        if v in taken: return False
    else:
        parities[par] = [v]
    for w in neigh:
        if phase_is_clifford(phases[w]) or w in taken: return False
