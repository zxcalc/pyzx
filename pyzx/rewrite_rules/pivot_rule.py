...
The boundary and gadget pivot variants are exposed via simplify.pivot_boundary_simp and
simplify.pivot_gadget_simp. Both expose two-vertex is_match/apply interfaces (built from
check_pivot_boundary/unsafe_pivot_boundary and check_pivot_gadget/unsafe_pivot_gadget) and
can be applied manually to a single (v, w) pair, e.g., from ZXLive. When run as a whole-graph
simplification (calling the rewrite as a function or via simp()), they fall back to the batch
matchers pivot_boundary_for_simp and pivot_gadget_for_simp, because the per-pair appliers
gadgetize w in place, which means applying them one at a time would let gadget vertices
created by an earlier pivot participate in later matches.
"""
...
def unsafe_pivot_boundary(
    ...
    # Use g.row(w) rather than g.rows()[w] so that graphs without explicit row
    # indices (which check_pivot_boundary doesn't require) fall back to -1
...
def unsafe_pivot_gadget(
    ...
    # Mirror the placement and qubit handling done by match_pivot_gadget, so
    # that pivot_gadget_simp.apply produces the same vertex metadata as
    # pivot_gadget_simp.simp. g.row is used (rather than g.rows()[v]) so that
    # graphs without explicit row indices fall back to -1 instead of raising
    # KeyError.
