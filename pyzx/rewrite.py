...
class RewriteSingleEdge(Rewrite[VT, ET]):
    applier: Callable[[BaseGraph[VT, ET], ET], bool]
    is_match: Callable[[BaseGraph[VT, ET], ET], bool]

    def __init__(self, is_match: Callable[[BaseGraph[VT, ET], ET], bool],
                 applier: Callable[[BaseGraph[VT, ET], ET], bool]) -> None:
        super().__init__()
        self.is_match = is_match
        self.applier = applier
        self.__doc__ = is_match.__doc__

    def apply(self, graph: BaseGraph[VT, ET], e: ET) -> None:
        if self.is_match(graph, e):
            self.applier(graph, e)


class RewriteSimpSingleEdge(RewriteSingleEdge[VT, ET]):
    simp_match: Optional[Callable[[BaseGraph[VT, ET], ET], bool]]

    def __init__(self, is_match: Callable[[BaseGraph[VT, ET], ET], bool],
                 applier: Callable[[BaseGraph[VT, ET], ET], bool],
                 simp_match: Optional[Callable[[BaseGraph[VT, ET], ET], bool]] = None) -> None:
        super().__init__(is_match, applier)
        self.simp_match = simp_match

    def find_all_matches(self, graph: BaseGraph[VT, ET]) -> Set[ET]:
        all_matches: Set[ET] = set()
        if self.simp_match is not None:
            match = self.simp_match
        else:
            match = self.is_match
