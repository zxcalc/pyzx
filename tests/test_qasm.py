...
class TestQASM(unittest.TestCase):
    ...
    def test_reset_to_graph(self):
        ...
        # Reset should create 2 X vertices (effect + state) for |0⟩.
        x_vertices = [v for v in g.vertices() if g.type(v) == VertexType.X]
        self.assertEqual(len(x_vertices), 2)
        for v in x_vertices:
    ...
    def test_measure_reset_measure_no_output(self):
        ...
        # q[0] ends with a measurement (no output), q[1] gets an output.
        self.assertEqual(len(g.outputs()), 1)
    ...
    def test_issue_345_circuit1_measure_reset(self):
        ...
        # Convert to graph.
        g = c.to_graph()
        self.assertEqual(len(g.inputs()), 8)
        # q[0] ends with measurement (no reset after), so 7 outputs.
        self.assertEqual(len(g.outputs()), 7)
    ...
    def test_issue_345_circuit2_ancilla_measure(self):
        ...
        # Convert to graph.
        g = c.to_graph()
        self.assertEqual(len(g.inputs()), 10)
        # Ancilla qubits 0-2 are measured (no output), data qubits 3-9 get outputs.
        self.assertEqual(len(g.outputs()), 7)
